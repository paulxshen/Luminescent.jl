function calibrate_mode(mode, ϵmode, dx, ; F=Float32, verbose=false, name="", kwargs...)
    ...
    m = [ModalMonitor(mode, [x, (L / 2)...], normal, tangent, L) for x = range(0, l0, 3) + source_margin + port_source_offset]
    s = ModalSource(t -> cispi(2t), mode, [source_margin, (L / 2)...], -normal, tangent, L;)
