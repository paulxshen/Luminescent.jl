...
@load "$(@__DIR__)/layout.json" static_mask sources ports designs λ dx ϵbase ϵclad ϵcore l w hbase hwg hclad
dx, = [dx,] / λ
...
function metrics(model, prob, dϵ=0; autodiff=true, history=nothing)
    ...
    # run simulation
    u = reduce((u, t) -> update(u, p, t, dx, dt, field_boundvals, source_instances;), 0:dt:T[1], init=deepcopy(u0))
...
@save "$(@__DIR__)/2d_model.json" model
# error()
