...
function make_geometry(model, configs, dϵ=0)#; make3d=false)
    ...
    mask = copy(mask_)
    ϵ = mask * (ϵcore + F(dϵ)) + (1 .- mask) * ϵclad
...
function metrics(model, configs, dϵ=0; autodiff=true, history=nothing)
    ...
    u, port_fluxes, cp = reduce(T[1]+dt:dt:T[2], init=(u, F(0), F(0))) do (u, port_fluxes, cp), t
        (maxwell_update(u, p, t, dx, dt, field_padding, source_instances),
            port_fluxes + dt * flux.((u,), monitor_instances,) / F(Δ[2]),
            cp + cispi(F(4t)) * mean.(port_fluxes) .* A / sqrt(F(nt)))
    end

    global port_fluxes
    # mean(vec(port_fluxes[2]) .* vec(power_profile)) * A
    port_powers = mean.(port_fluxes) .* A
    port_mode_powers = [vec(a) ⋅ vec(power_profile) / norm(a) for a = port_fluxes] .* port_powers
...
function score(v1, v2)
    @show tp1, cp1 = v1
    @show tp2, cp2 = v2
    @show a = angle(cp2 / cp1) / 2
    -(v1[1] + v2[1]) / mp0 / 2 - abs(a) / dϕ0
end
...
T = F.(T)
Δ = F.(Δ)
ϵsub, ϵcore, ϵclad, dx, dt, dϵ, dϕ0, mp0 = F.((ϵsub, ϵcore, ϵclad, dx, dt, dϵ, dϕ0, mp0))
